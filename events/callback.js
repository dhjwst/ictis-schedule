            ...
            switch(query.data) {
                case "main":
                    getTable(result.group).then(async () => {
                        let board = []
                        isFavorite(query.message.chat.id, result.name).then(async flag => {
                            if (flag) {
                                board = [
                ...
                case query.data.match(/^[а-яА-Я0-9\/\.]/)?.input:
                    ...
                    getGroup(query.data).then(async group => {
                        getTable(group.group).then(async table => {
                            let boards = []
                            isFavorite(query.message.chat.id, group.name).then(async flag => {
                                ...
                                const opts = {
                                    ...
                                    reply_markup: {
                                        resize_keyboard: true,
                                        inline_keyboard: boards
                                    }
                                };

                                await bot.editMessageText(`*📚 Расписание - ${group.name} - ${table.week} неделя*\n\n`, opts)
                        })
                    })
                })
